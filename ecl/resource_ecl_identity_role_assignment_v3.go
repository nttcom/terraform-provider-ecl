...
func resourceIdentityRoleAssignmentV3Read(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Retrieved ECL role assignment: %#v", roleAssignment)
...
func getRoleAssignment(identityClient *eclcloud.ServiceClient, d *schema.ResourceData) (roles.RoleAssignment, error) {
	domainID, projectID, groupID, userID, roleID := extractRoleAssignmentID(d.Id())
...
// Role assignments have no ID in Enterprise Cloud.
