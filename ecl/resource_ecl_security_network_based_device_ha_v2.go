...
func resourceSecurityNetworkBasedDeviceHAV2() *schema.Resource {
	return &schema.Resource{
		Create: resourceSecurityNetworkBasedDeviceHAV2Create,
		Read:   resourceSecurityNetworkBasedDeviceHAV2Read,
		Update: resourceSecurityNetworkBasedDeviceHAV2Update,
		Delete: resourceSecurityNetworkBasedDeviceHAV2Delete,
...
func resourceSecurityNetworkBasedDeviceHAV2Create(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	...
	return resourceSecurityNetworkBasedDeviceHAV2Read(d, meta)
}

func resourceSecurityNetworkBasedDeviceHAV2Read(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	...
	pClient, err := config.securityPortalV2Client(GetRegion(d, config))
	if err != nil {
...
func resourceSecurityNetworkBasedDeviceHAV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("locale") || d.HasChange("operating_mode") || d.HasChange("license_kind") {
		...
		resourceSecurityNetworkBasedDeviceHAV2UpdateOrderAPIPart(d, meta)
	}

	if d.HasChange("port") {
		log.Printf("[DEBUG] Start changing device by portal api.")
		resourceSecurityNetworkBasedDeviceHAV2UpdatePortalAPIPart(d, meta)
	}

	return resourceSecurityNetworkBasedDeviceHAV2Read(d, meta)
}

func resourceSecurityNetworkBasedDeviceHAV2Delete(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.securityOrderV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating ECL security order client: %s", err)
