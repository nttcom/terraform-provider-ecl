			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"ecl_db_configuration_v1.basic", "name", "basic"),
					resource.TestCheckResourceAttr(
						"ecl_db_configuration_v1.basic", "configuration.2.name", "max_connections"),
					resource.TestCheckResourceAttr(
						"ecl_db_configuration_v1.basic", "configuration.2.value", "200"),
				),
			},
		},
	})
...
func testAccCheckDatabaseV1ConfigurationDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "ecl_db_configuration_v1" {
			continue
		}
