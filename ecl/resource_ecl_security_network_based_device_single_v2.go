...
func resourceSecurityNetworkBasedDeviceSingleV2() *schema.Resource {
...
func resourceSecurityNetworkBasedDeviceSingleV2Create(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	...
	return resourceSecurityNetworkBasedDeviceSingleV2Read(d, meta)
}

func resourceSecurityNetworkBasedDeviceSingleV2Read(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
...
func resourceSecurityNetworkBasedDeviceSingleV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("locale") || d.HasChange("operating_mode") || d.HasChange("license_kind") {
		...
		resourceSecurityNetworkBasedDeviceSingleV2UpdateOrderAPIPart(d, meta)
	}

	if d.HasChange("port") {
		log.Printf("[DEBUG] Start changing device by portal api.")
		resourceSecurityNetworkBasedDeviceSingleV2UpdatePortalAPIPart(d, meta)
	}

	return resourceSecurityNetworkBasedDeviceSingleV2Read(d, meta)
}

func resourceSecurityNetworkBasedDeviceSingleV2Delete(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.securityOrderV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating ECL security order client: %s", err)
