...
func TestAccIdentityV3UserDataSource_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccIdentityUserV3DataSource_user(userName, userPassword),
			},
			resource.TestStep{
				Config: testAccIdentityUserV3DataSource_basic(userName, userPassword),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIdentityUserV3DataSourceID("data.ecl_identity_user_v3.user_1"),
					resource.TestCheckResourceAttr(
						"data.ecl_identity_user_v3.user_1", "name", userName),
					resource.TestCheckResourceAttr(
						"data.ecl_identity_user_v3.user_1", "domain_id", "default"),
					resource.TestCheckResourceAttr(
						"data.ecl_identity_user_v3.user_1", "enabled", "true"),
					testAccCheckIdentityUserV3DataSourceDefaultProjectID(
						"data.ecl_identity_user_v3.user_1", "ecl_identity_project_v3.project_1"),
				),
			},
		},
	})
...
func testAccIdentityUserV3DataSource_user(name, password string) string {
	return fmt.Sprintf(`
	%s
...
`, testAccIdentityProjectV3DataSource_project(fmt.Sprintf("%s_project", name), acctest.RandString(20)), name, password)
}

func testAccIdentityUserV3DataSource_basic(name, password string) string {
	return fmt.Sprintf(`
	%s

	data "ecl_identity_user_v3" "user_1" {
      name = "${ecl_identity_user_v3.user_1.name}"
	}
`, testAccIdentityUserV3DataSource_user(name, password))
}
