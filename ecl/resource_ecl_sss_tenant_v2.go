...
func resourceSSSTenantV2() *schema.Resource {
	return &schema.Resource{
		Create: resourceSSSTenantV2Create,
		Read:   resourceSSSTenantV2Read,
		//Update: resourceSSSTenantV2Update,
		Delete: resourceSSSTenantV2Delete,
...
func resourceSSSTenantV2Create(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	...
	return resourceSSSTenantV2Read(d, meta)
}

func resourceSSSTenantV2Read(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sssV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating ECL sss client: %s", err)
    ...
    d.Set("workspace_id", tenant.WorkspaceID)

	log.Printf("[DEBUG] resourceSSSTenantV2Read Succeeded")

	return nil
}

/*
func resourceSSSTenantV2Update(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sssV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating ECL sss client: %s", err)
...
func resourceSSSTenantV2Delete(d *schema.ResourceData, meta interface{}) error {
    return fmt.Errorf("Terraform does not allow Tenant to be deleted." +
			          "Please run terraform state rm to remove the resource from under Terraform management.")
	/*
	config := meta.(*Config)
	client, err := config.sssV2Client(GetRegion(d, config))
	if err != nil {
