...
func resourceSSSApprovalRequestV2() *schema.Resource {
	return &schema.Resource{
		Create: resourceSSSApprovalRequestV2Create,
		Read:   resourceSSSApprovalRequestV2Read,
		Delete: resourceSSSApprovalRequestV2Delete,
...
func resourceSSSApprovalRequestV2Create(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	...
	return resourceSSSApprovalRequestV2Read(d, meta)
}

func resourceSSSApprovalRequestV2Read(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sssV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("error creating ECL sss client: %w", err)
...
func resourceSSSApprovalRequestV2Delete(d *schema.ResourceData, meta interface{}) error {
	return nil
}
