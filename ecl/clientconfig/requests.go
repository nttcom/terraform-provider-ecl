...
// AuthOptions creates a eclcloud.AuthOptions structure with the
...
// v2auth creates a v2-compatible eclcloud.AuthOptions struct.
func v2auth(cloud *Cloud, opts *ClientOpts) (*eclcloud.AuthOptions, error) {
...
// v3auth creates a v3-compatible eclcloud.AuthOptions struct.
func v3auth(cloud *Cloud, opts *ClientOpts) (*eclcloud.AuthOptions, error) {
	// Environment variable overrides.
	envPrefix := "OS_"
	if opts != nil && opts.EnvPrefix != "" {
		envPrefix = opts.EnvPrefix
	...
	// eclcloud properly authenticates with a token. This involves
	...
	if ao.IdentityEndpoint == "" {
		err := eclcloud.ErrMissingInput{Argument: "auth_url"}
		return nil, err
	}
...
func AuthenticatedClient(opts *ClientOpts) (*eclcloud.ProviderClient, error) {
	...
	return ecl.AuthenticatedClient(*ao)
}
...
func NewServiceClient(service string, opts *ClientOpts) (*eclcloud.ServiceClient, error) {
	...
	case "identity":
		...
		case "v3", "3":
			return ecl.NewIdentityV3(pClient, eo)
		default:
			return nil, fmt.Errorf("invalid identity API version")
		}
	case "image":
		return ecl.NewImageServiceV2(pClient, eo)
	case "network":
		return ecl.NewNetworkV2(pClient, eo)
	case "object-store":
		return ecl.NewObjectStorageV1(pClient, eo)
	case "orchestration":
		return ecl.NewOrchestrationV1(pClient, eo)
	case "volume":
		volumeVersion := "2"
		...
		case "v2", "2":
			return ecl.NewComputeVolumeV2(pClient, eo)
		default:
