...
// 2. Current directory.
// 3. unix-specific user_config_dir (~/.config/ecl/clouds.yaml)
// 4. unix-specific site_config_dir (/etc/ecl/clouds.yaml)
//
...
func findAndReadYAML(yamlFile string) ([]byte, error) {
	...
	// unix user config directory: ~/.config/ecl.
	...
	if homeDir != "" {
		filename := filepath.Join(homeDir, ".config/ecl/"+yamlFile)
		if ok := fileExists(filename); ok {
			return ioutil.ReadFile(filename)
		}
	}

	// unix-specific site config directory: /etc/ecl.
	if ok := fileExists("/etc/ecl/" + yamlFile); ok {
		return ioutil.ReadFile("/etc/ecl/" + yamlFile)
	}
