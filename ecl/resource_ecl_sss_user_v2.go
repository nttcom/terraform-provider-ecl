...
func resourceSSSUserV2() *schema.Resource {
	return &schema.Resource{
		Create: resourceSSSUserV2Create,
		Read:   resourceSSSUserV2Read,
		Update: resourceSSSUserV2Update,
		Delete: resourceSSSUserV2Delete,
...
func resourceSSSUserV2Create(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	...
	return resourceSSSUserV2Read(d, meta)
}

func resourceSSSUserV2Read(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sssV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating ECL sss client: %s", err)
	...
	log.Printf("[DEBUG] resourceSSSUserV2Read Succeeded")

	return nil
}

func resourceSSSUserV2Update(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sssV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating ECL sss client: %s", err)
...
func resourceSSSUserV2Delete(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sssV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating ECL sss client: %s", err)
