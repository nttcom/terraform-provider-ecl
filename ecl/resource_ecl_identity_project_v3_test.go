			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPtr(
						"ecl_identity_project_v3.project_1", "name", &project.Name),
					resource.TestCheckResourceAttrPtr(
						"ecl_identity_project_v3.project_1", "description", &project.Description),
					resource.TestCheckResourceAttr(
						"ecl_identity_project_v3.project_1", "domain_id", "default"),
					resource.TestCheckResourceAttr(
			...
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"ecl_identity_project_v3.project_1", "domain_id", "default"),
					resource.TestCheckResourceAttr(
						"ecl_identity_project_v3.project_1", "enabled", "false"),
					resource.TestCheckResourceAttr(
						"ecl_identity_project_v3.project_1", "is_domain", "false"),
				),
			},
...
func testAccCheckIdentityV3ProjectDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "ecl_identity_project_v3" {
			continue
		}
