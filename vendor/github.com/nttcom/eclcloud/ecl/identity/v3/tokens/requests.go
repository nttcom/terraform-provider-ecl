...
type AuthOptions struct {
	...
	// AllowReauth should be set to true if you grant permission for Eclcloud
	// to cache your credentials in memory, and to allow Eclcloud to attempt
...
func (opts *AuthOptions) ToTokenV3CreateMap(scope map[string]interface{}) (map[string]interface{}, error) {
	eclcloudAuthOpts := eclcloud.AuthOptions{
	...
	return eclcloudAuthOpts.ToTokenV3CreateMap(scope)
}

// ToTokenV3CreateMap builds a scope request body from AuthOptions.
func (opts *AuthOptions) ToTokenV3ScopeMap() (map[string]interface{}, error) {
	scope := eclcloud.AuthScope(opts.Scope)
	...
	return eclcloudAuthOpts.ToTokenV3ScopeMap()
}
...
func subjectTokenHeaders(c *eclcloud.ServiceClient, subjectToken string) map[string]string {
...
func Create(c *eclcloud.ServiceClient, opts AuthOptionsBuilder) (r CreateResult) {
	...
	resp, err := c.Post(tokenURL(c), b, &r.Body, &eclcloud.RequestOpts{
...
func Get(c *eclcloud.ServiceClient, token string) (r GetResult) {
	resp, err := c.Get(tokenURL(c), &r.Body, &eclcloud.RequestOpts{
...
func Validate(c *eclcloud.ServiceClient, token string) (bool, error) {
	resp, err := c.Head(tokenURL(c), &eclcloud.RequestOpts{
		MoreHeaders: subjectTokenHeaders(c, token),
...
func Revoke(c *eclcloud.ServiceClient, token string) (r RevokeResult) {
	_, r.Err = c.Delete(tokenURL(c), &eclcloud.RequestOpts{
		MoreHeaders: subjectTokenHeaders(c, token),
	})
	return
}
